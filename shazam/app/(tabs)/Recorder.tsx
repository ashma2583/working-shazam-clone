...
export default function Recorder() {
  ...
  const handlePrediction = async () => {
    ...
    if (uri) {
      ...
      const predict_endpoint = "http://35.0.107.63:5003/predict";
  ...
  async function addSongToDatabase(song_url: any) {
    ...
    // TODO: type in your server address here
    const add_endpoint = "http://35.0.107.63:5003/add_song";
    ...
    fetch(add_endpoint, {
        ...
        .then((data) => {
          ...
          // Assuming server sends back { message: "Success" } or similar
          Alert.alert("Success", data.message || "Song added successfully!");
          setText("");
        })
        .catch((error) => {
          console.error("Error adding song:", error);
          Alert.alert("Error", `Could not add song: ${error.message}`);
        })
        .finally(() => {
          setAddingSong(false); // Stop the loading indicator regardless of success or failure
          });
  }
